...
/** DELETE /api/plantao-acompanhamento/agenda?id=... */
async function excluir(req: VercelRequest, res: VercelResponse): Promise<void> {
  const id = param(req, 'id');
  if (!id) return erro(res, 400, 'Informe o id do registro.');

  const { error } = await getSupabase().from(TABELA_AGENDA_INDEVIDA).delete().eq('id', id);
